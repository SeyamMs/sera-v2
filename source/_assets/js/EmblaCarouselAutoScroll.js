export const addPlayBtnListeners = (emblaApi) => {
	...
	emblaApi.on("autoScroll:play", togglePlayBtnState).on("autoScroll:stop", togglePlayBtnState).on("reInit", togglePlayBtnState)

	return () => {
		// playBtn.removeEventListener("click", onPlayBtnClick)
		emblaApi.off("autoScroll:play", togglePlayBtnState).off("autoScroll:stop", togglePlayBtnState).off("reInit", togglePlayBtnState)
	}
}
